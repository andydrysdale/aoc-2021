...
class Vent:
    ...
    def getCoords(self):
        ...
        if deltaX == 0 and deltaY >= 0:          # vertical, y2 higher
            for y in range(self.y1, self.y2 + 1):
                coords.append([self.x1, y])
        elif deltaX == 0 and deltaY < 0:         # vertical, y1 higher
            for y in range(self.y2, self.y1 + 1):
                coords.append([self.x1, y])
        elif deltaY == 0 and deltaX >= 0:        # horizontal, x2 higher
            for x in range(self.x1, self.x2 + 1):
                coords.append([x, self.y1])
        elif deltaY == 0 and deltaX < 0:         # horizontal, x1 higher
            for x in range(self.x2, self.x1 + 1):
                coords.append([x, self.y1])     
        elif deltaX == deltaY and deltaX > 0:    # diagonal, x2 and y2 higher
            for i in range(self.x2 - self.x1 + 1):
                coords.append([self.x1 + i, self.y1 + i])  
        elif deltaX == deltaY and deltaX < 0:    # diagonal, x1 and y1 higher
            for i in range(self.x1 - self.x2 + 1):
                coords.append([self.x2 + i, self.y2 + i])  
        elif deltaX == (deltaY * -1) and deltaX > 0:    # diagonal, x2 higher and y1 higher
            for i in range(self.x2 - self.x1 + 1):
                coords.append([self.x1 + i, self.y1 - i])  
        elif deltaX == (deltaY * -1) and deltaX < 0:    # diagonal, x1 higher and y2 higher
            for i in range(self.x1 - self.x2 + 1):
                coords.append([self.x1 - i, self.y1 + i])  
        return coords
